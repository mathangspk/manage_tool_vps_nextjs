...
export default function CgsatPage() {
  ...
  const handleSaveItem = async (e) => {
    ...
    try {
      if (editingItem) {
        await cgsatsApi.updateCgsat(editingItem._id, payload);
      } else {
        await cgsatsApi.createCgsat(payload);
      }
      setModalOpen(false);
      fetchItems();
    } catch (error) {
      console.error('Error saving CGSAT:', error);
  ...
  const handleDeleteItem = async (item) => {
    if (!confirm('Bạn có chắc chắn muốn xóa phiếu Thay đổi GSAT này?')) return;
    try {
      await cgsatsApi.deleteCgsat(item._id);
      fetchItems();
    } catch (error) {
      console.error('Error deleting CGSAT:', error);
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md border border-slate-100 overflow-hidden animate-scale-in">
            ...
            <div className="h-14 flex items-center justify-between px-6 border-b border-slate-100 bg-slate-50">
              <h4 className="font-bold text-slate-800 text-sm uppercase">
                {editingItem ? 'Sửa Phiếu Thay Đổi GSAT' : 'Lấy Số GSAT Mới (Tạo Phiếu)'}
              </h4>
              ...
              <div className="p-6 space-y-4">
                ...
                {editingItem && (
                  <div className="text-xs font-semibold text-slate-500">
                    Mã số phiếu: <span className="text-slate-800 font-bold">{editingItem.PCGSAT}</span>
                  </div>
                )}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
                    placeholder="Mô tả nội dung thay đổi giám sát an toàn..."
                  />
                </div>
